...
# Fallback values
username = "avi"
fullname = "Avi Vashishta"

profile_path = os.path.join(HERE, "..", "data", "profile.json")
if os.path.exists(profile_path):
    try:
        with open(profile_path, "r") as f:
            profile_data = json.load(f)
        username = profile_data.get("username", username)
        fullname = profile_data.get("fullname", fullname)
    except Exception as e:
        print(f"Error reading profile.json: {e}")

if len(sys.argv) <= 2:
    OUT = os.path.join(HERE, "..", f"{username}-ascii.svg")
...
status_text_raw = f"{username}@github:~$ whoami {fullname}"
cursor_offset = int(len(status_text_raw) * 5.8) + 2
parts.append(f'<text x="{PAD}" y="{status_y:.1f}" fill="{TITLE_TEXT}" font-size="13">'
             f'{html.escape(username)}@github:~$ whoami <tspan fill="{INK}">{html.escape(fullname)}</tspan></text>')
parts.append(f'<rect x="{PAD+cursor_offset}" y="{status_y-12:.1f}" width="8" height="14" fill="{INK}">'
