...
if os.path.exists(profile_path):
    try:
        ...
        host_title = profile_data.get("host_title", host_title)
        ROWS = [tuple(r) for r in profile_data.get("info_card_rows", ROWS)]
...
parts.append(f'<text x="{W/2}" y="{TITLEBAR_H/2 + 4}" fill="{MUTED}" font-size="12" '
             f'text-anchor="middle">{esc(username)}@github: ~$ {esc(host_title)}</text>')
